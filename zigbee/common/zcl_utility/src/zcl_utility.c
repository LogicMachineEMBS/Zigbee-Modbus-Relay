...
esp_err_t esp_zcl_utility_add_ep_basic_manufacturer_info(esp_zb_ep_list_t *ep_list, uint8_t endpoint_id, zcl_basic_manufacturer_info_t *info)
{
    ...
    ESP_ERROR_CHECK(esp_zb_cluster_add_attr(basic_cluster, ESP_ZB_ZCL_CLUSTER_ID_BASIC,
        0xF000, ESP_ZB_ZCL_ATTR_TYPE_U8 , ESP_ZB_ZCL_ATTR_ACCESS_READ_WRITE, &info->modbus_id));
    
    ESP_ERROR_CHECK(esp_zb_cluster_add_attr(basic_cluster, ESP_ZB_ZCL_CLUSTER_ID_BASIC,
        0xF001, ESP_ZB_ZCL_ATTR_TYPE_U32 , ESP_ZB_ZCL_ATTR_ACCESS_READ_WRITE, &info->modbus_baudrate));

    ESP_ERROR_CHECK(esp_zb_cluster_add_attr(basic_cluster, ESP_ZB_ZCL_CLUSTER_ID_BASIC,
        0xF002, ESP_ZB_ZCL_ATTR_TYPE_U8 , ESP_ZB_ZCL_ATTR_ACCESS_READ_WRITE, &info->modbus_parity));
